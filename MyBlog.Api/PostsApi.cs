...
public static class PostsApi
{
    ...
    {
        routes.MapGet("/api/posts", async (AppDbContext db) =>
        {
        ...
        routes.MapGet("/api/posts/{id}", async Task<Results<Ok<PostWithoutComments>, NotFound>> (AppDbContext db, int id) =>
        {
        ...
        routes.MapPost("/api/posts", async Task<Results<Created, BadRequest>> (AppDbContext db, CreatePost model) =>
        {
        ...
        routes.MapDelete("/api/posts/{id}", async Task<Results<NoContent, NotFound>> (AppDbContext db, int id) =>
        {
